#!/usr/bin/env node
/**
 * Browser smoke test for the apps/web client.
 *
 * Exercises the current preview flow against a live stack: create a Vault
 * (Argon2id derivation in WASM), verify recovery controls are unavailable,
...
try {
  ...
  await page.locator('#lock-routes').waitFor({ timeout: 10000 });
  ...
  await page.locator('#view-app').waitFor({ timeout: 60000 });
  ...
  log('✓ Vault created without exposing the incomplete recovery flow');
  ...
  // Saving leaves the post-save panel open — close it so rows are clickable.
  // Press and verify instead of assuming a fixed animation delay.
...
console.log('\n✓ Browser smoke test passed (derive, seal, store, decrypt, reveal, delete, lock).');
} catch (error) {
