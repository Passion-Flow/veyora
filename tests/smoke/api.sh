...
# Usage: ./tests/smoke/api.sh [API_URL]
#
...
# 4. Retrieve record
echo -n "4. GET record... "
GET_CT=$(curl -s -m 5 "$API/records/$RECORD_ID?vault=$VAULT_ID" 2>/dev/null | \
...
LIST_COUNT=$(curl -s -m 5 "$API/records?vault=$VAULT_ID" 2>/dev/null | \
...
DEL_STATUS=$(curl -s -m 5 -o /dev/null -w "%{http_code}" \
  -X DELETE "$API/records/$RECORD_ID?vault=$VAULT_ID" \
  -H "content-type: application/json" \
...
TOMB=$(curl -s -m 5 "$API/records/$RECORD_ID?vault=$VAULT_ID" 2>/dev/null | \
