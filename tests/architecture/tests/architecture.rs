...
fn is_safety_core(relative: &Path) -> bool {
    ...
    s.starts_with("packages/config/") || s.starts_with("packages/contracts-rust/")
}
...
fn files_under(root: &Path) -> Vec<PathBuf> {
    ...
    let mut pending = vec![
        root.join("services").to_path_buf(),
        root.join("packages").to_path_buf(),
    ];
    while let Some(directory) = pending.pop() {
        for entry in fs::read_dir(directory).expect("read service directory") {
            let path = entry.expect("read backend entry").path();
            if path.file_name().is_some_and(|name| name == "target") {
                continue;
            }
            // The security kernel is a separate review boundary with its own
            // workspace and test surface; it is not backend source.
            if path
                .file_name()
                .is_some_and(|name| name == "security-kernel")
            {
                continue;
            }
            if path.is_dir() {
...
for package in document.get("packages", []):
    ...
    for dependency in package.get("dependencies", []):
        ...
        if not Path(path).resolve().is_relative_to(root):
            raise SystemExit("dependency outside the repository root")
...
fn allowed_standard_library_references(
    ...
    for (index, token) in tokens.iter().enumerate() {
        ...
        let marker = if relative == "packages/config/src/resolved.rs"
            && starts_with(remaining, &["std", "::", "fmt", ";"])
        {
            "fmt"
        } else if relative == "packages/config/src/resolved.rs"
            && starts_with(
                remaining,
                &[
                    "std",
        ...
        {
            "collections-group"
        } else if relative == "packages/contracts-rust/src/lib.rs"
            && starts_with(
                remaining,
...
fn workspace_metadata_and_package_dependency_edges_are_closed() {
    ...
    assert_structured_metadata(&metadata, &repository_root());
}
...
fn safety_core_rust_targets_match_the_closed_allowlist() {
    ...
    let safety_core_expected = [
        ...
        "packages/config/src/generated.rs",
        "packages/config/src/value.rs",
        "packages/config/src/resolved.rs",
        "packages/config/src/role_plan.rs",
        "packages/config/src/bootstrap_plan.rs",
...
fn production_targets_have_no_file_process_or_network_runtime_surface() {
        ...
        .filter(|path| !path.components().any(|part| part.as_os_str() == "tests"))
    {
        ...
        if !relative.starts_with("packages/contracts-rust/") {
            assert!(
    ...
    assert_eq!(
        ...
        [
            "packages/config/src/resolved.rs:collections-group".to_owned(),
            "packages/config/src/resolved.rs:fmt".to_owned(),
            "packages/contracts-rust/src/lib.rs:btree-map".to_owned(),
            "packages/contracts-rust/src/lib.rs:btree-map".to_owned(),
            "packages/contracts-rust/src/lib.rs:btree-map".to_owned(),
            "packages/contracts-rust/src/lib.rs:btree-map".to_owned(),
            "packages/contracts-rust/src/lib.rs:btree-map".to_owned(),
    ...
    for path in files {
        ...
        let in_functional_migrations = relative
            .starts_with("packages/storage/postgres/migrations/")
            || relative.starts_with("packages/storage/sqlite/migrations/");
        assert!(
            name != "build.rs"
                && name != "Dockerfile"
...
fn runtime_surface_parser_rejects_grouped_fully_qualified_and_aliased_std_access() {
    let path = Path::new("packages/config/src/resolved.rs");
    for hostile in [
        "use std::{fmt, net};",
...
fn service_shells_are_exact_immediate_exit_targets() {
    // worker/validator/backup/restore remain inert immediate-exit shells
    // until each is built out. The api and migrator services are functional.
    for service in [] as [&str; 0] {
        let root = repository_root().join("services").join(service);
...
fn migrator_service_is_a_functional_migration_runner() {
    let main = fs::read_to_string(repository_root().join("services/migrator/src/main.rs"))
...
fn worker_service_is_a_functional_background_processor() {
    let main = fs::read_to_string(repository_root().join("services/worker/src/main.rs"))
...
fn backup_service_is_a_functional_snapshot_exporter() {
    let main = fs::read_to_string(repository_root().join("services/backup/src/main.rs"))
...
fn restore_service_is_a_functional_snapshot_importer() {
    let main = fs::read_to_string(repository_root().join("services/restore/src/main.rs"))
...
#[test]
fn validator_service_is_a_functional_ciphertext_validator() {
    let main = fs::read_to_string(repository_root().join("services/validator/src/main.rs"))
        .expect("read validator main");
    assert!(
        main.contains("validate_record") && main.contains("stdin"),
        "validator must be a functional ciphertext validator"
    );
}

#[test]
fn api_service_is_a_functional_http_server() {
    let main = fs::read_to_string(repository_root().join("services/api/src/main.rs"))
        .expect("read api main");
    assert!(
