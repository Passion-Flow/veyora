...
fn generated_canonical_matrix_compiles_to_five_deny_by_default_roles() {
    ...
    assert!(plans.for_service(ServiceNameV1::Validator).is_none());
...
#[test]
fn rejects_validator_authority_mutations() {
    let mutations: [fn(&mut ServiceCapabilitiesV1); 6] = [
        ...
        |matrix| matrix.services.validator.jobs.push("hostile-job".into()),
        |matrix| {
            matrix
                .services
                .validator
                .routes
                .push("hostile-route".into())
        },
    ];
    for mutate in mutations {
        let mut hostile = canonical_service_capabilities();
        mutate(&mut hostile);
        let error = ServiceAuthorizationPlans::compile(&hostile)
            .expect_err("validator authority must fail closed");
        assert_eq!(error.code(), AuthorizationPlanErrorCode::CapabilityMismatch);
    }
}
