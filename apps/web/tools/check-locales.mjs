...
for (const [file, catalog] of catalogs) {
  ...
  if (!problems.length) {
    ...
    for (const key of referenceKeys) {
      if (!(key in catalog.messages)) problems.push(`missing key "${key}"`);
    }
    for (const key of referenceKeys) {
      const message = catalog.messages[key];
      if (!message) continue;
  ...
  if (problems.length) {
    // Every shipped locale must have full key parity (PRD LANG-005);
    // missing keys are release-blocking, not a fallback warning.
    hardErrors += 1;
