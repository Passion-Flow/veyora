...
/** Characters valid in an RFC 4648 Base32 secret (case-folded, unpadded). */
const BASE32_RE = /^[A-Z2-7]+$/;

/**
 * Normalize user-provided TOTP material (ITEM-005). Accepts either a raw
 * Base32 secret or an `otpauth://` URI; safe whitespace and grouping
 * dashes are ignored. Returns `{ok: true, value}` with the normalized
 * uppercase Base32 secret, or `{ok: false, reason}` with a stable reason
 * (`uri`, `type`, `base32`) the caller maps to catalog copy. Never throws.
 */
export function normalizeTotpSecret(raw) {
  ...
  if (input.toLowerCase().startsWith('otpauth://')) {
    ...
    if (!value || !BASE32_RE.test(value)) return { ok: false, reason: 'uri' };
    return { ok: true, value };
  }
  const value = input.replace(/[\s-]/g, '').toUpperCase();
  if (!BASE32_RE.test(value)) return { ok: false, reason: 'base32' };
  return { ok: true, value };
}
