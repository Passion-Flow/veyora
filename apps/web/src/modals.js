...
function renderEntryFields(entry) {
  ...
  $('#entry-fields').innerHTML = `
      <div class="error-summary hidden" id="entry-error-summary" role="alert"
           tabindex="-1"></div>`
    + primary.map(def =>
      fieldHtml(def, entry ? (entry[def.k] || '') : '')).join('')
    + (more.length ? `
      ...
      <div id="more-fields-wrap" class="fgrid${expanded ? '' : ' hidden'}">
        ${more.map(def => fieldHtml(def, entry ? (entry[def.k] || '') : '')).join('')}
      </div>` : '');
...
async function saveEntry() {
  ...
  try {
    if (state.editingId) {
      ...
      defs.forEach(def => {
        ...
        candidate[def.k] = def.k === 'totpSecret' ? totp.value : read(def.k);
      });
      try {
      ...
      defs.forEach(def => {
        if (def.k === 'name') return;
        entry[def.k] = def.k === 'totpSecret' ? totp.value : read(def.k);
      });
      try {
  ...
  renderTabs();
  renderTable();
  // The drawer may still be open on this entry (edit path); refresh it so
  // the visible revision and updated time match what was just saved.
