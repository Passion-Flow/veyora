...
/**
 * Render a stable API error code as localized prose (LANG-003).
 *
 * The API returns only stable ASCII codes with fixed English messages;
 * the client owns user-facing localization. Unknown or missing codes
 * fall back to the generic `apiError.default` message.
 */
export function apiErrorMessage(code) {
  if (code && resolve(`apiError.${code}`)) return t(`apiError.${code}`);
  return t('apiError.default');
}
