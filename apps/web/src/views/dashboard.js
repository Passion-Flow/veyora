...
import { backupOverlayHtml, wireBackupOverlay, bindBackupRenderers } from './backup.js';
import { regenerateOverlayHtml, wireRegenerateOverlay } from './backup.js';
import { kitOverlayHtml, wireKitOverlay } from './backup.js';
import { supportBundleOverlayHtml } from './support-bundle.js';
import { firstSuccess } from '../core/telemetry.js';
import { vaultIdentity, isDesktopRuntime } from '../data/diagnostics.js';
...
export function visibleEntries() {
  ...
  else if (state.nav === 'tags') {
    // The Tags scope lists tagged entries; an active tag chip narrows to it.
    list = list.filter(entry => Array.isArray(entry.tags) && entry.tags.length > 0);
    if (state.tagFilter) list = list.filter(entry => entry.tags.includes(state.tagFilter));
  ...
  $('#root').innerHTML = `
    <div id="view-app" class="on">
      ...
      <div class="table-area">
        ...
        <div id="tag-chips" class="filter-bar" aria-label="${t('nav.tags')}"></div>
      ...
      <div class="overlay" id="ov-backup" role="dialog" aria-modal="true" aria-labelledby="backup-title">
        ${backupOverlayHtml()}
      </div>
      <div class="overlay" id="ov-regen" role="dialog" aria-modal="true" aria-labelledby="regen-title">
        ${regenerateOverlayHtml()}
      </div>
      <div class="overlay" id="ov-kit" role="dialog" aria-modal="true" aria-labelledby="kit-title">
        ${kitOverlayHtml()}
      </div>
...
function wireDashboard() {
  wireBackupOverlay();
  wireRegenerateOverlay();
  wireKitOverlay();
  bindBackupRenderers(renderTabs, renderTable);
...
/** Distinct tags across the vault, sorted (the Tags tab's filter set). */
export function distinctTags() {
  const set = new Set();
  vault.entries.forEach(entry => (entry.tags || []).forEach(tag => set.add(tag)));
  return [...set].sort();
}

/** The Tags scope renders its tag chips above the table (PRD 7.1). */
function renderTagChips() {
  const host = document.getElementById('tag-chips');
  if (!host) return;
  const tags = distinctTags();
  // The filter-bar host stays hidden until it has content (CSS .on gate).
  host.classList.add('on');
  host.innerHTML = tags.length
    ? `${t('tags.filterLabel')} ` + tags.map(tag =>
      `<button class="filter-chip${state.tagFilter === tag ? ' on' : ''}" data-tag="${tag}"
         aria-pressed="${state.tagFilter === tag}">${tag}</button>`).join('')
    : `<span class="micro">${t('tags.none')}</span>`;
  host.querySelectorAll('[data-tag]').forEach(chip =>
...
export function renderTable() {
  ...
  // The Tags scope renders its chip set; other scopes clear the host.
  const tagChips = document.getElementById('tag-chips');
  if (state.nav === 'tags') {
    renderTagChips();
  } else if (tagChips) {
    tagChips.innerHTML = '';
    tagChips.classList.remove('on');
  }
  ...
  const pendingBadgeLabel = isDesktopRuntime() ? 'table.pendingSyncDesktop' : 'table.pendingSync';
  // Windowed rendering (PERF-F-001/PERF-006): large vaults must not render
  // every row on each save/search — the visible window keeps the
  // acknowledgment path fast, and Show more expands in bounded steps.
  const WINDOW_STEP = 500;
  if (!state.rowWindow) state.rowWindow = WINDOW_STEP;
  if (state.query.trim() || state.nav !== 'all') state.rowWindow = WINDOW_STEP;
  const windowSize = Math.min(state.rowWindow, list.length);
  const rendered = list.slice(0, windowSize);
  body.innerHTML = rendered.map(entry => {
    ...
    const badges = [];
    if (pendingSyncIds.has(entry.id)) badges.push(`<span class="badge-pending" title="${t(pendingBadgeLabel)}">${icon('clock', 10)}</span>`);
  ...
  if (state.nav === 'trash' && list.length > 0) {
    // ITEM-008: permanent deletion with a concrete, armed confirmation
    // that names the item count and irreversibility (ITEM-009 early purge).
    const count = list.length;
    body.insertAdjacentHTML('beforeend', `
      <div class="table-empty">
        <button class="btn" id="btn-purge-trash" aria-describedby="purge-note">
          ${icon('trash', 14)}${t('trash.purgeBtn', { count })}</button>
        <div class="micro" id="purge-note">${t('trash.purgeNote')}</div>
      </div>`);
    const purge = $('#btn-purge-trash');
    purge.onclick = async () => {
      if (!purge.dataset.armed) {
        purge.dataset.armed = '1';
        purge.classList.add('btn-danger-confirm');
        purge.innerHTML = `${icon('alert', 14)}${t('trash.purgeConfirm', { count })}`;
        setTimeout(() => {
          if (purge.isConnected && purge.dataset.armed) {
            delete purge.dataset.armed;
            purge.classList.remove('btn-danger-confirm');
            purge.innerHTML = `${icon('trash', 14)}${t('trash.purgeBtn', { count })}`;
          }
        }, 8000);
        return;
      }
      try {
        const purged = await recordSync.purgeVaultTrash();
        state.trashEntries = [];
        state.nav = 'trash';
        toast(t('trash.purgeDone', { count: purged }), 'check', null, 'trash.purgeDone');
        renderTabs();
        renderTable();
      } catch (thrown) {
        toast(apiErrorMessage((thrown && thrown.code) || 'PM-NETWORK-UNREACHABLE'), 'alert');
      }
    };
  }
  if (list.length > windowSize) {
    const remaining = list.length - windowSize;
    body.insertAdjacentHTML('beforeend', `
      <div class="table-empty">
        <button class="btn" id="btn-more-rows" aria-label="${t('table.showMore', { count: Math.min(WINDOW_STEP, remaining) })}">
          ${icon('chevronDown', 14)}${t('table.showMore', { count: Math.min(WINDOW_STEP, remaining) })}</button>
        <div class="micro">${t('table.windowedCount', { shown: windowSize, total: list.length })}</div>
      </div>`);
    const more = $('#btn-more-rows');
    more.onclick = () => {
      state.rowWindow = windowSize + WINDOW_STEP;
      renderTable();
    };
  }
}
