...
import { t, setLocale, formatDate, formatNumber, apiErrorMessage } from '../src/i18n/index.js';
...
const CATALOGS = {
  en: {
    ...
    messages: {
      ...
      'apiError.default': { text: 'Something went wrong.' },
      'apiError.PM-STORE-NOT-FOUND': { text: 'Record not found.' },
    },
  },
  'zh-CN': {
    schema_version: 1, locale: 'zh-CN', direction: 'ltr',
    messages: {
      'apiError.PM-STORE-NOT-FOUND': { text: '记录不存在。' },
    },
  },
  ru: {
...
test('api error codes localize through the client catalog (LANG-003)', async () => {
  await setLocale('zh-CN', { load });
  assert.equal(apiErrorMessage('PM-STORE-NOT-FOUND'), '记录不存在。');
});

test('unknown api error codes fall back to the generic message', async () => {
  await setLocale('en', { load });
  assert.equal(apiErrorMessage('PM-UNKNOWN-CODE'), 'Something went wrong.');
