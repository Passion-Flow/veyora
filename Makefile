...
.PHONY: help check check-web build build-wasm test test-kernel test-backend test-wasm-runtime test-browser run run-web run-db migrate worker backup restore sandbox docker-build docker-up docker-down clean
...
test: check check-web test-kernel test-backend ## Run all dependency-free source checks
