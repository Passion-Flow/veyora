...
#   ./deploy/tools/deploy.sh                          # Interactive setup
#   ./deploy/tools/deploy.sh --domain vault.example.com  # Non-interactive
#
# Prerequisites:
#   - Docker Engine + Docker Compose
#   - OpenSSL (for password generation)
#   - TLS certificates (Let's Encrypt or your CA)
#   - deploy/compose/.env created from deploy/compose/.env.example
set -euo pipefail
...
if [ -n "$DOMAIN" ]; then
  ...
  echo "  For production: ./deploy/tools/deploy.sh --domain your.domain.com"
fi
...
if [ "$HEALTH" = "200" ] && [ "$WEB" = "200" ]; then
  ...
  echo "       cd deploy/compose && docker compose --profile backup up -d backup"
else
  echo "⚠ Some services may not be ready. Check logs:"
  echo "  cd deploy/compose && docker compose logs"
fi
