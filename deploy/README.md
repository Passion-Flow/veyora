...
Docker Compose. `compose/` holds the canonical topology file and the
environment template; `containers/` holds the image definitions for the
Rust services and registry mirrors; `gateway/` and `web/` hold the Envoy
and static web client delivery configuration.
...
`.env` is the local startup file. Copy it from `compose/.env.example` for a
...
annotated `compose/.env.example` documents the full set.

## Directory layout

| Path | Purpose |
| --- | --- |
| `compose/compose.yaml` | Canonical topology: PostgreSQL, migrator, API, worker, gateway, web, backup |
| `compose/.env.example` | Annotated environment template |
...
validator) are built from the Dockerfiles under deploy/containers/ at the repository
...
- Consider `VEYORA_API_CORS_ORIGINS` and `VEYORA_API_RATE_LIMIT` from
  the hardening section of `compose/.env.example`.
