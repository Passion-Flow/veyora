...
fn build_store(store_kind: &str) -> Result<Arc<dyn OpaqueStore>, String> {
    match store_kind {
        "postgres" => {
            ...
            // Apply the schema when the process holds the DDL role. Under
            // the DB-004 DML-only role the migrator service has already
            // applied it, so migrate() is correctly denied — fall back to
            // verifying the schema instead of refusing to start.
            if let Err(migrate_error) = store.migrate() {
                store.verify_schema().map_err(|_| {
                    format!(
                        "records migration failed ({migrate_error}); \
                         cannot start without a valid schema"
                    )
                })?;
            }
            Ok(Arc::new(store))
