...
use backend_persistence::{
    GenericEncryptedRecordV1, OpaqueStore, RecordSummary, StoreError, utc_now_iso,
};
...
pub fn app(state: AppState) -> Router {
    Router::new()
        ...
        .route("/vault/rekey", post(rekey_vault).options(cors_ok))
...
/// Per-boot prefix (Unix epoch seconds at startup, hex) so request IDs stay
/// unique across process restarts while remaining bounded-cardinality
/// (DEP-014: correlation without high-cardinality labels).
static REQUEST_ID_BOOT: std::sync::OnceLock<u64> = std::sync::OnceLock::new();

fn request_id_boot() -> u64 {
    *REQUEST_ID_BOOT.get_or_init(|| {
...
async fn auth_middleware(
    ...
    let mut response = error_response(axum::http::StatusCode::UNAUTHORIZED, "PM-API-UNAUTHORIZED");
    response
        .headers_mut()
...
async fn batch_put_records(
    State(state): State<AppState>,
    payload: Result<Json<Vec<RecordDto>>, axum::extract::rejection::JsonRejection>,
    ...
    let scope = dtos
        .first()
        .map(|dto| dto.vault_id.clone())
        .filter(|vault| !vault.is_empty())
        .ok_or(ApiError::BadBody)?;
    if dtos.iter().any(|dto| dto.vault_id != scope) {
        return Err(ApiError::BadBody);
    }
    let ids: Vec<String> = dtos.iter().map(|dto| dto.record_id.clone()).collect();
    let expected: Vec<Option<u64>> = dtos.iter().map(|dto| dto.expected_prior_revision).collect();
    let records: Vec<_> = dtos.into_iter().map(RecordDto::into_record).collect();
    let requested = records.len();
    // DATA-007: one transaction — any row failure rolls the whole batch
    // back, so the response either reports every requested row committed or
    // an error with nothing applied (API-010: requested is never a success
    // count).
    let revisions = state
        .run(move |store| store.put_batch(records, expected))
        .await
        .map_err(ApiError::from)?;
    let committed = revisions.len();
    let results = ids
        .into_iter()
        .zip(revisions)
        .map(|(record_id, revision)| BatchResult {
            record_id,
            revision,
        })
        .collect();
...
/// Exact batch outcome counts (API-010): `committed` is the number of rows
/// the transaction actually applied; a failed batch never reaches this
/// shape (it fails through the error envelope with nothing applied).
...
///
/// A failed dependency answers `503` so orchestrators stop routing traffic;
/// `/healthz` stays process-alive-only and keeps answering `200` so the
/// container is not restarted for a database outage (API-006).
async fn ready_check(
    State(state): State<AppState>,
) -> Result<(StatusCode, Json<ReadyStatus>), ApiError> {
    // Reachability probe only (API-006): an empty scope lists nothing, so
    // success proves the store answers queries without exposing any row.
    match state.run(|store| store.list("", 1, 0)).await {
        Ok(_) => Ok((
            StatusCode::OK,
            Json(ReadyStatus {
                ready: true,
...
/// Default and strict listing bounds (DATA-006).
const DEFAULT_LIST_LIMIT: usize = 500;
const MAX_LIST_LIMIT: usize = 5_000;
/// Strict maximum for the `?embed=bodies` hydration round trip; larger
/// vaults page through the summaries endpoint instead.
const MAX_EMBED_BODIES: usize = 500;
const HEADER_TRUNCATED: &str = "x-truncated";
...
pub struct ListQuery {
    ...
    /// Declared vault scope for the listing (DATA-002): the server returns
    /// only that Vault's rows. Until authenticated pairing principals exist
    /// (ADR 0005, Proposed), the scope is client-declared; it is still
    /// enforced on every query rather than returning global rows.
    pub vault: Option<String>,
    /// Page size (DATA-006): clamped to `limit <= MAX_LIST_LIMIT`; larger
    /// requests are malformed rather than silently honored.
    pub limit: Option<usize>,
    /// Page offset (DATA-006).
...
async fn list_records(
    State(state): State<AppState>,
    query: Result<axum::extract::Query<ListQuery>, axum::extract::rejection::QueryRejection>,
) -> Result<axum::response::Response, ApiError> {
    // Query-string failures also carry a stable code (DIAG-003).
    let axum::extract::Query(query) = query.map_err(|_| ApiError::BadQuery)?;
    let embed_bodies = query.embed.as_deref() == Some("bodies");
    let vault = required_vault(query.vault.as_deref())?;
    // DATA-006: every listing is bounded and paginated. Limits above the
    // strict maximum are malformed requests, never silently honored.
    ...
    if embed_bodies {
        // The single-round-trip hydration path has its own strict maximum;
        // larger vaults must page through the summaries endpoint.
        let probe_limit = MAX_EMBED_BODIES + 1;
        let records = state
            .run(move |store| store.list_bodies(&vault, probe_limit, offset))
            .await
            .map_err(ApiError::from)?;
        if records.len() > MAX_EMBED_BODIES {
            return Err(ApiError::BadQuery);
        }
        let dtos: Vec<RecordDto> = records.into_iter().map(RecordDto::from_record).collect();
        return Ok(([(HEADER_TRUNCATED, "false")], Json(dtos)).into_response());
    }
    let summaries = state
        .run(move |store| store.list(&vault, limit, offset))
        .await
        .map_err(ApiError::from)?;
    // A full page means another page may exist: page-aligned truncation is
    // explicit, never silent.
    let truncated = summaries.len() == limit;
    let marker = if truncated { "true" } else { "false" };
    let dtos: Vec<RecordSummaryDto> = summaries.into_iter().map(RecordSummaryDto::from).collect();
    Ok(([(HEADER_TRUNCATED, marker)], Json(dtos)).into_response())
}
...
async fn get_record(
    ...
    let vault = required_vault(query.vault.as_deref())?;
    let record = state
        .run(move |store| store.get(&vault, &id))
        .await
        .map_err(ApiError::from)?;
    Ok(Json(RecordDto::from_record(record)))
}

/// The declared vault scope every record operation requires (DATA-002).
/// Missing or empty scopes are malformed requests, not implicit globals.
fn required_vault(vault: Option<&str>) -> Result<String, ApiError> {
    vault
        .filter(|value| !value.is_empty())
        .map(str::to_string)
        .ok_or(ApiError::BadQuery)
}
...
async fn put_record(
    State(state): State<AppState>,
    Path(id): Path<String>,
    payload: Result<Json<RecordDto>, axum::extract::rejection::JsonRejection>,
) -> Result<Response, ApiError> {
    // Malformed bodies must fail through the stable-code envelope (DIAG-003).
    let Json(dto) = payload.map_err(|_| ApiError::BadBody)?;
    if dto.record_id != id {
    ...
    let vault = record.vault_id.clone();
        ...
        .await
    {
        ...
        Err(store_error) => {
            let error = conflict_with_authoritative_revision(
                &state,
                &vault,
                &id_for_conflict,
                ApiError::from(store_error),
            )
            .await;
            return Err(error);
        }
...
async fn tombstone_record(
    ...
    Path(id): Path<String>,
    query: Result<axum::extract::Query<ListQuery>, axum::extract::rejection::QueryRejection>,
    payload: Result<Json<TombstoneRequest>, axum::extract::rejection::JsonRejection>,
) -> Result<Response, ApiError> {
    // Malformed queries/bodies must fail through the stable-code envelope
    // (DIAG-003).
    let axum::extract::Query(query) = query.map_err(|_| ApiError::BadQuery)?;
    let vault = required_vault(query.vault.as_deref())?;
    let Json(body) = payload.map_err(|_| ApiError::BadBody)?;
    let id_for_conflict = id.clone();
    let vault_for_conflict = vault.clone();
    let revision = match state
        .run(move |store| store.tombstone(&vault, &id, body.expected_prior_revision))
        .await
    {
        Ok(revision) => revision,
        Err(store_error) => {
...
async fn purge_tombstoned(
    ...
    let axum::extract::Query(query) = query.map_err(|_| ApiError::BadQuery)?;
    let vault = required_vault(query.vault.as_deref())?;
    // User-initiated purge: every tombstone stamped at or before this
    // instant goes; the cutoff form keeps one purge implementation for the
    // client ceremony and the worker's retention policy alike (DATA-008).
    let cutoff = backend_persistence::utc_now_epoch();
    let purged = state
        .run(move |store| store.purge_tombstoned_before(&vault, cutoff))
        .await
        .map_err(ApiError::from)?;
...
/// Request body of the atomic vault rekey (Master Password change).
#[derive(Debug, Serialize, Deserialize)]
...
/// Atomic Master Password change at the storage layer: every record moves
/// from the query scope to `to_vault` in one transaction and the old scope
/// disappears. Any failure — an occupied target slot, an invalid row, or a
/// store error — rolls the whole operation back, so the vault stays whole
/// under the old scope and the client can safely retry.
async fn rekey_vault(
    State(state): State<AppState>,
    query: Result<axum::extract::Query<ListQuery>, axum::extract::rejection::QueryRejection>,
    payload: Result<Json<RekeyRequest>, axum::extract::rejection::JsonRejection>,
) -> Result<Json<RekeyResult>, ApiError> {
    let axum::extract::Query(query) = query.map_err(|_| ApiError::BadQuery)?;
    let vault = required_vault(query.vault.as_deref())?;
    let request = payload.map_err(|_| ApiError::BadBody)?;
    if request.to_vault.trim().is_empty() || request.to_vault == vault {
        return Err(ApiError::BadBody);
    }
    // Rows arrive sealed for the target scope only; a mixed-scope body is
    // malformed, never partially applied.
    let RekeyRequest {
        to_vault,
        records: dtos,
    } = request.0;
    let records: Vec<_> = dtos.into_iter().map(RecordDto::into_record).collect();
    if records.is_empty() || records.iter().any(|record| record.vault_id != to_vault) {
        return Err(ApiError::BadBody);
    }
    let moved = state
        .run(move |store| store.rekey_vault(&vault, &to_vault, records))
        .await
        .map_err(ApiError::from)?;
    Ok(Json(RekeyResult { moved }))
}
...
///
/// Errors serialize as `{"error":{"code":…,"message":…}}`. The stable ASCII
/// code is the contract; the fixed English message is a debugging aid and
/// is never localized (LANG-003). Clients render localized prose from
/// their own locale catalogs keyed by the code.
#[derive(Debug)]
pub enum ApiError {
    Store(StoreError),
    /// A compare-and-set conflict whose response carries the server's
    /// current committed revision (DATA-005).
    ConflictAtRevision(u64),
    RouteMismatch,
    BadBody,
    BadQuery,
}
...
impl ApiError {
    fn status_and_code(&self) -> (StatusCode, &'static str) {
        match self {
            ...
            Self::Store(StoreError::Conflict) | Self::ConflictAtRevision(_) => {
                (StatusCode::CONFLICT, "PM-STORE-CONFLICT")
            }
            ...
            Self::BadQuery => (StatusCode::BAD_REQUEST, "PM-API-BAD-QUERY"),
        }
    }
}
...
/// Map a store conflict from a put/tombstone into a DATA-005 response that
/// carries the server-authoritative current revision when the row still
/// exists; a vanished row keeps the bare conflict code.
async fn conflict_with_authoritative_revision(
    state: &AppState,
    vault: &str,
    record_id: &str,
    error: ApiError,
) -> ApiError {
    if !matches!(error, ApiError::Store(StoreError::Conflict)) {
        return error;
    }
    let vault = vault.to_string();
    let record_id = record_id.to_string();
    match state
        .run(move |store| store.current_revision(&vault, &record_id))
        .await
    {
        Ok(current) => ApiError::ConflictAtRevision(current),
        Err(_) => ApiError::Store(StoreError::Conflict),
    }
}
...
mod tests {
    ...
    /// Store double whose every operation fails as unavailable, used to prove
    /// readiness reflects the storage dependency (API-006).
    struct UnavailableStore;

    impl OpaqueStore for UnavailableStore {
        fn put(
            &self,
            _record: GenericEncryptedRecordV1,
            _expected_prior_revision: Option<u64>,
        ...
        fn get(
            ...
            _vault_id: &str,
            _record_id: &str,
        ) -> Result<GenericEncryptedRecordV1, StoreError> {
            Err(StoreError::StoreUnavailable)
        }
        fn list(
            &self,
    ...
    fn utc_format_matches_known_instants() {
        use backend_persistence::format_utc;
        // Unix epoch and known instants (leap-year coverage included).
    ...
    #[test]
    fn request_ids_carry_a_per_boot_prefix() {
        // The boot-prefixed form stays pure hex so it remains a valid
        // header value and correlates across restarts (DEP-014).
        let req_id = format!(
            "{:08x}{:012x}",
            request_id_boot(),
            REQUEST_ID_COUNTER.fetch_add(0, Ordering::Relaxed)
        );
        assert!(
            req_id.chars().all(|c| c.is_ascii_hexdigit()),
            "request id must be hex: {req_id}"
        );
    }
    ...
    async fn put_then_get_round_trips_opaque_record() {
        ...
        let put_response = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/record-aaaa?vault=1010101010101010101010101010101f")
    ...
    async fn cas_conflict_on_stale_expected_revision() {
        ...
        let _ = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/record-cccc?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(body))
                    .unwrap(),
            )
    ...
    async fn list_returns_records() {
        ...
        let response = router
            .oneshot(
                axum::http::Request::builder()
                    .uri("/records?vault=1010101010101010101010101010101f")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn delete_tombstones_a_record() {
        ...
        let _ = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/record-tomb?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(body))
                    .unwrap(),
            )
    ...
    #[tokio::test]
    async fn readyz_reports_store_outage_as_service_unavailable() {
        ...
        // Readiness fails with 503 so orchestrators stop routing traffic.
        let response = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
        ...
        assert!(!status.ready);
        assert_eq!(status.record_count, 0);
        // Health stays process-alive-only: a database outage must not make an
        // alive process report dead (and get restarted) (API-006).
        let response = router
            .oneshot(
                axum::http::Request::builder()
                    .uri("/healthz")
    ...
    async fn batch_put_creates_multiple_records() {
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let bytes = response.into_body().collect().await.unwrap().to_bytes();
        let batch: BatchResponse = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(batch.requested, 3);
        // API-010: committed equals what the transaction applied, which for
        // an all-or-nothing batch is every requested row or none.
        assert_eq!(batch.committed, 3);
        assert_eq!(batch.results.len(), 3);
        assert!(batch.results.iter().all(|r| r.revision >= 1));
    ...
    async fn oversized_body_is_rejected() {
        ...
        let response = router
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/too-big?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
    ...
    #[tokio::test]
    async fn vault_rekey_moves_every_record_atomically() {
        let router = app_with_store();
        let old_vault = "1010101010101010101010101010101f";
        let new_vault = "3030303030303030303030303030303f";
        // Seed two records in the old scope.
        for id in ["rekey-a", "rekey-b"] {
            let body = serde_json::to_vec(&dto(id, 1)).unwrap();
            let response = router
                .clone()
                .oneshot(
                    axum::http::Request::builder()
                        .method("PUT")
                        .uri(format!("/records/{id}?vault={old_vault}"))
                        .header("content-type", "application/json")
                        .body(axum::body::Body::from(body))
                        .unwrap(),
                )
        ...
        // Rekey into the new scope: rows arrive sealed for the target only.
        let rekey_rows: Vec<RecordDto> = ["rekey-a", "rekey-b"]
            .into_iter()
            .map(|id| {
                let mut row = dto(id, 1);
                row.vault_id = new_vault.to_string();
                row
            })
            .collect();
        let response = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri(format!("/vault/rekey?vault={old_vault}"))
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(
                        serde_json::to_vec(&RekeyRequest {
                            to_vault: new_vault.to_string(),
                            records: rekey_rows,
                        })
                        .unwrap(),
                    ))
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let bytes = response.into_body().collect().await.unwrap().to_bytes();
        let result: RekeyResult = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(result.moved, 2);
        // The old scope is empty; the new scope holds both rows.
        let list = |vault: String| {
            let router = router.clone();
            async move {
                let response = router
                    .oneshot(
                        axum::http::Request::builder()
                            .uri(format!("/records?vault={vault}"))
                            .body(axum::body::Body::empty())
                            .unwrap(),
                    )
                    .await
                    .unwrap();
                let bytes = response.into_body().collect().await.unwrap().to_bytes();
                serde_json::from_slice::<Vec<RecordSummaryDto>>(&bytes).unwrap()
            }
        };
        assert!(list(old_vault.to_string()).await.is_empty());
        assert_eq!(list(new_vault.to_string()).await.len(), 2);
        // A rekey into an occupied target refuses without partial effects.
        let mut occupied = dto("rekey-a", 1);
        occupied.vault_id = new_vault.to_string();
        let response = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
    ...
    async fn purge_tombstoned_returns_count() {
        ...
        let _ = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/purge-test?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(body))
                    .unwrap(),
            )
        ...
        let _ = router
            ...
            .await
            .unwrap();
        // Purge at the current epoch.
        let response = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri("/vault/purge?vault=1010101010101010101010101010101f")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
        ...
        // The endpoint purges at the current epoch: every tombstone now
        // (DATA-008). The stamp-and-cutoff boundary lives in the adapter
        // contract suite; the store here is the in-memory reference.
        assert_eq!(result.purged, 1, "a tombstoned record is purged");
        // The purged row is gone for this vault; live rows elsewhere are
        // untouched by the scoped sweep.
        let gone = router
            .oneshot(
                axum::http::Request::builder()
                    .method("GET")
    ...
    async fn error_body_is_json_with_stable_code() {
        let response = app_with_store()
            .oneshot(
                axum::http::Request::builder()
                    .uri("/records/does-not-exist?vault=1010101010101010101010101010101f")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
    ...
    #[tokio::test]
    async fn error_message_is_english_regardless_of_accept_language() {
        // LANG-003: the API never localizes prose. An Accept-Language header
        // must not change the message and must not add Content-Language.
        for header in ["zh-CN,zh;q=0.9,en;q=0.8", "ja", "xx-YY"] {
            let response = app_with_store()
                .oneshot(
                    axum::http::Request::builder()
                        .uri("/records/does-not-exist?vault=1010101010101010101010101010101f")
    ...
    async fn oversized_body_error_is_json_envelope() {
        ...
        assert_eq!(
            body.error.message,
            error_catalog::message("PM-API-BODY-TOO-LARGE")
        );
    }

    #[tokio::test]
    async fn malformed_body_rejection_uses_stable_code() {
        // DIAG-003: axum extractor rejections must surface through the
        // stable-code envelope, never the default plain-text rejection.
        for (method, uri) in [
            (
                "PUT",
                "/records/bad-json?vault=1010101010101010101010101010101f",
            ),
            (
                "DELETE",
                "/records/bad-json?vault=1010101010101010101010101010101f",
            ...
            (
                "POST",
                "/records/batch?vault=1010101010101010101010101010101f",
            ),
        ] {
            let response = app_with_store()
                .oneshot(
    ...
    #[tokio::test]
    async fn malformed_query_rejection_uses_stable_code() {
        // A repeated `embed` field fails Query deserialization, which must
        // surface as the stable PM-API-BAD-QUERY envelope (DIAG-003).
        let response = app_with_store()
            .oneshot(
                axum::http::Request::builder()
                    .uri("/records?embed=a&embed=b")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
        ...
        let bytes = response.into_body().collect().await.unwrap().to_bytes();
        let body: ErrorBody = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(body.error.code, "PM-API-BAD-QUERY");
        assert_eq!(
            body.error.message,
            error_catalog::message("PM-API-BAD-QUERY")
        );
    }
    ...
    async fn put_revision_response_is_json() {
        ...
        let response = router
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/record-json?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(body))
                    .unwrap(),
            )
    ...
    async fn list_embed_bodies_returns_full_records_in_one_round_trip() {
        ...
        let _ = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/embed-aaaa?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(body))
                    .unwrap(),
            )
        ...
        let response = router
            ...
            .oneshot(
                axum::http::Request::builder()
                    .uri("/records?embed=bodies&vault=1010101010101010101010101010101f")
                    .body(axum::body::Body::empty())
                    .unwrap(),
            )
            .await
    ...
    #[tokio::test]
    async fn colliding_record_ids_stay_scoped_to_their_vaults() {
        // E2E-005 collision scope / DATA-001: the same record ID written for
        // two different Vaults is two independent records; each listing and
        // read stays inside its own Vault boundary.
        let router = app_with_store();
        let mut vault_a = dto("shared-id", 1);
        vault_a.vault_id = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".to_string();
        let mut vault_b = dto("shared-id", 1);
        vault_b.vault_id = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb".to_string();
        for dto in [&vault_a, &vault_b] {
            ...
            let response = router
                ...
                .oneshot(
                    axum::http::Request::builder()
                        .method("PUT")
                        .uri(format!("/records/{}?vault={}", dto.record_id, dto.vault_id))
                        .header("content-type", "application/json")
                        .body(axum::body::Body::from(body))
                        .unwrap(),
                )
        ...
        for vault in [
            ...
            let bytes = response.into_body().collect().await.unwrap().to_bytes();
            let summaries: Vec<RecordSummaryDto> = serde_json::from_slice(&bytes).unwrap();
            assert_eq!(summaries.len(), 1, "vault {vault} sees only its own row");
            // Reading the shared ID through an unknown vault's scope
            // misses: the row exists, but not in that scope.
            let response = router
                .clone()
                .oneshot(
                    axum::http::Request::builder()
                        .uri("/records/shared-id?vault=cccccccccccccccccccccccccccccccc")
                        .body(axum::body::Body::empty())
                        .unwrap(),
                )
                .await
    ...
    #[tokio::test]
    async fn batch_conflict_rolls_the_whole_batch_back() {
        // DATA-007: one conflicting row aborts the entire batch — the store
        // must not hold the other rows afterward.
        let router = app_with_store();
        let seed = serde_json::to_vec(&dto("batch-seed", 1)).unwrap();
        let response = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/batch-seed?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
            ...
            .unwrap();
        assert_eq!(response.status(), StatusCode::CREATED);
        // Second row claims a stale revision for the seeded record.
        let mut stale = dto("batch-seed", 1);
        stale.expected_prior_revision = Some(99);
        let batch = vec![dto("batch-fresh", 1), stale];
        let body = serde_json::to_vec(&batch).unwrap();
        let response = router
            .clone()
            .oneshot(
        ...
        // Nothing from the batch was applied: the fresh row is absent and
        // the seeded record keeps its original revision.
        let response = router
            .oneshot(
                axum::http::Request::builder()
                    .uri("/records?vault=1010101010101010101010101010101f")
        ...
        let summaries: Vec<RecordSummaryDto> = serde_json::from_slice(&bytes).unwrap();
        assert!(
            !summaries.iter().any(|s| s.record_id == "batch-fresh"),
            "the fresh row must not survive a rolled-back batch"
        );
        let seeded = summaries
            .iter()
            .find(|s| s.record_id == "batch-seed")
            .expect("seeded record survives");
        assert_eq!(seeded.revision, 1, "the seeded row is untouched");
    }

    #[tokio::test]
    async fn batch_with_mixed_vault_scopes_is_malformed() {
        // DATA-004: a batch declaring two vault scopes is rejected whole.
        let router = app_with_store();
        let mut foreign = dto("batch-foreign", 1);
        foreign.vault_id = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb".to_string();
        let batch = vec![dto("batch-local", 1), foreign];
        let body = serde_json::to_vec(&batch).unwrap();
        let response = router
            .oneshot(
                axum::http::Request::builder()
        ...
        let bytes = response.into_body().collect().await.unwrap().to_bytes();
        let error: ErrorBody = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(error.error.code, "PM-API-BAD-BODY");
    }

    #[tokio::test]
    async fn listings_are_bounded_and_paginated() {
        // DATA-006: limit/offset page the listing, oversized limits are
        // malformed, and a full page carries the truncation marker.
        let router = app_with_store();
        let vault = "1010101010101010101010101010101f";
        let mut batch = Vec::new();
        for index in 0..3 {
            batch.push(dto(&format!("page-{index:02}"), 1));
        }
        let body = serde_json::to_vec(&batch).unwrap();
        let response = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("POST")
                    .uri(format!("/records/batch?vault={vault}"))
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(body))
                    .unwrap(),
            )
        ...
        let list = |router: axum::Router, query: String| {
            Box::pin(async move {
                router
                    .oneshot(
                        axum::http::Request::builder()
                            .uri(format!("/records?vault={vault}{query}"))
                            .body(axum::body::Body::empty())
                            .unwrap(),
                    )
                    .await
        ...
        // Page of two: bounded result with the truncation marker.
        let response = list(router.clone(), "&limit=2&offset=0".into()).await;
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(response.headers().get("x-truncated").unwrap(), "true");
        let bytes = response.into_body().collect().await.unwrap().to_bytes();
        let page: Vec<RecordSummaryDto> = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(page.len(), 2);
        // The next page is the final row: no marker.
        let response = list(router.clone(), "&limit=2&offset=2".into()).await;
        assert_eq!(response.headers().get("x-truncated").unwrap(), "false");
        let bytes = response.into_body().collect().await.unwrap().to_bytes();
        let page: Vec<RecordSummaryDto> = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(page.len(), 1);
        // Oversized and zero limits are malformed, never silently honored.
        for bad in ["&limit=0", "&limit=5001"] {
            let response = list(router.clone(), bad.into()).await;
            assert_eq!(response.status(), StatusCode::BAD_REQUEST, "{bad}");
            let bytes = response.into_body().collect().await.unwrap().to_bytes();
            let error: ErrorBody = serde_json::from_slice(&bytes).unwrap();
            assert_eq!(error.error.code, "PM-API-BAD-QUERY");
        }
    }

    #[tokio::test]
    async fn conflict_response_carries_the_authoritative_revision() {
        // DATA-005: a stale compare-and-set answers 409 with the server's
        // current committed revision so clients resolve without guessing.
        let router = app_with_store();
        let vault = "1010101010101010101010101010101f";
        let seed = serde_json::to_vec(&dto("conflict-row", 1)).unwrap();
        let response = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri(format!("/records/conflict-row?vault={vault}"))
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(seed))
                    .unwrap(),
            )
            ...
            .unwrap();
        assert_eq!(response.status(), StatusCode::CREATED);
        // Advance the server revision to 3, then write with a stale CAS.
        for revision in [Some(1), Some(2)] {
            let mut dto = dto("conflict-row", 2);
            dto.expected_prior_revision = revision;
            let body = serde_json::to_vec(&dto).unwrap();
            let response = router
                .clone()
                .oneshot(
        ...
        let mut stale = dto("conflict-row", 2);
        stale.expected_prior_revision = Some(1);
        ...
        let error: ErrorBody = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(error.error.code, "PM-STORE-CONFLICT");
        let parameters = error.error.parameters.expect("conflict carries parameters");
        assert_eq!(
            parameters.get("current_revision"),
            Some(&3),
    ...
    #[tokio::test]
    async fn missing_vault_scope_is_a_malformed_query() {
        // DATA-002: a record operation without a declared vault scope is a
        // stable-code rejection, never an implicit global read.
        let router = app_with_store();
        let response = router
            .oneshot(
                axum::http::Request::builder()
    ...
    #[tokio::test]
    async fn metrics_never_leak_record_or_vault_identifiers() {
        // API-007: metrics must carry no Vault IDs, item IDs, paths, or
        // ciphertext material — only bounded-cardinality counters.
        let router = app_with_store();
        let body = serde_json::to_vec(&dto("metrics-canary-record", 1)).unwrap();
        let _ = router
            .clone()
            .oneshot(
                axum::http::Request::builder()
                    .method("PUT")
                    .uri("/records/metrics-canary-record?vault=1010101010101010101010101010101f")
                    .header("content-type", "application/json")
                    .body(axum::body::Body::from(body))
                    .unwrap(),
            )
        ...
        let text = String::from_utf8(bytes.to_vec()).unwrap();
        for canary in [
            "metrics-canary-record",
            "1010101010101010101010101010101f", // dto vault_id
            "records/metrics",
        ] {
            assert!(!text.contains(canary), "metrics leaked {canary}: {text}");
        }
    }
