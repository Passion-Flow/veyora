...
The root Rust workspace contains the ciphertext API, storage adapters, database
...
| `services/validator/` | Bounded record-format validation without network or database access |
| `packages/storage/persistence/` | Storage interface and in-memory implementation |
| `packages/storage/postgres/` | PostgreSQL implementation and migrations |
| `packages/config/` | Explicit configuration model and generated registry projection |
| `packages/contracts-rust/` | Checked-in Rust representations of shared contracts |
...
See [deploy/compose/.env.example](../deploy/compose/.env.example) for the Compose-facing values.
