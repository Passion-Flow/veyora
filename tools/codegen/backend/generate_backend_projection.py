...
SERVICES = ("gateway", "api", "worker", "web", "validator", "migrator", "backup", "restore")
CAPABILITY_FIELDS = (
    "routes",
    "database_operations",
    "secret_files",
...
def validate_capabilities(capabilities: dict[str, Any]) -> dict[str, Any]:
    ...
    for service in SERVICES:
        ...
        if service != "validator" and "stdin" in value:
            raise ProjectionError(f"only validator may define stdin")
    return services
...
def render(
    ...
    return f"""// @generated by tools/codegen/backend/generate_backend_projection.py.
