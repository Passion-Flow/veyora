...
#   REGISTRY=your-registry.example.com NAMESPACE=your-account VERSION=v1.0.0 ./tools/release/publish-containers.sh
#
...
#   UPSTREAM_MIRROR=docker.m.daocloud.io ./tools/release/publish-containers.sh
#
# Prerequisites:
...
UPSTREAM_IMAGES=(
  ...
  "rust:1.93.1-slim-bookworm@sha256:8f8609d448e821fbc0e44241bc5ca4ce49663cc6306ff1a17f655a0e2a7cd084"
...
if [ "${SKIP_MIRRORS:-0}" != "1" ]; then
  for index in "${!FOUNDATIONS[@]}"; do
    ...
    docker buildx build \
      ...
      --tag "$image" \
      -f deploy/containers/mirror/Dockerfile \
      --push \
      .
    echo "  ✓ Pushed $image"
  done
...
SERVICES=("api" "worker" "migrator" "backup" "restore" "validator")
...
for svc in "${SERVICES[@]}"; do
  ...
  docker buildx build \
    ...
    --tag "$IMAGE" \
    -f "deploy/containers/$svc/Dockerfile" \
    --push \
    .
  echo "  ✓ Pushed $IMAGE"
done
...
echo ""
echo "Images:"
for svc in postgres nginx envoy rust debian api worker migrator backup restore validator web gateway; do
  image="$REGISTRY/$NAMESPACE/veyora-$svc:$VERSION"
