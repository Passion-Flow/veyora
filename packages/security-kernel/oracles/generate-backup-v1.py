...
GENERATOR_LINK_COMMAND = (
    ...
    'packages/security-kernel/oracles/secretstream-v1.c "$INSTALL_DIR/lib/libsodium.a" '
    '-pthread -o "$WORK_DIR/secretstream-v1"'
)
CHECKER_LINK_COMMAND = (
    '"$CC" -std=c11 -O2 -Wall -Wextra -Werror -I"$INSTALL_DIR/include" '
    'packages/security-kernel/oracles/check-secretstream-v1.c "$INSTALL_DIR/lib/libsodium.a" '
    '-pthread -o "$WORK_DIR/check-secretstream-v1"'
)
GENERATION_COMMAND = (
    'SODIUM_DIST_DIR="$SODIUM_DIST_DIR" '
    "python3 packages/security-kernel/oracles/generate-backup-v1.py "
    "--output packages/security-kernel/vectors/backup-v1.json"
)
UPSTREAM_GENERATOR = {
...
def build_minisign_verifier(
    ...
    return binary, {
        "minisign_verifier_source_ref": "packages/security-kernel/oracles/minisign-verifier/src/main.rs",
...
def build_fixture(
    ...
    for index in range(message_count):
        ...
        relative_sidecar = f"packages/security-kernel/vectors/backup-v1/{case_name}/frame-{index:03d}.ciphertext.bin"
        if (
            len(aad) == 0
...
def build_corpus(
    ...
    oracle_directory = repository / "packages/security-kernel/oracles"
    ...
    oracle_evidence: dict[str, object] = {
        ...
        **verifier_evidence,
        ...
        "generator_source_ref": "packages/security-kernel/oracles/secretstream-v1.c",
        "generator_source_sha256": sha256(oracle_directory / "secretstream-v1.c"),
        "generator_binary_sha256": sha256(generator),
        "checker_source_ref": "packages/security-kernel/oracles/check-secretstream-v1.c",
        "checker_source_sha256": sha256(oracle_directory / "check-secretstream-v1.c"),
        "checker_binary_sha256": sha256(checker),
        "transcriber_source_ref": "packages/security-kernel/oracles/generate-backup-v1.py",
    ...
    project_generator = {
        ...
        "source_ref": "packages/security-kernel/oracles/secretstream-v1.c",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("packages/security-kernel/vectors/backup-v1.json"),
    )
    parser.add_argument("--check", action="store_true")
    arguments = parser.parse_args()
    repository = Path(__file__).resolve().parents[2]
    output = arguments.output if arguments.output.is_absolute() else repository / arguments.output
    if not output.is_relative_to(repository / "packages/security-kernel/vectors"):
        raise RuntimeError("output must remain under packages/security-kernel/vectors")
