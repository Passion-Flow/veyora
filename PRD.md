...
| ---: | --- | --- |
| 1 | ✅ `Complete` | Live release audit executed with owner-authenticated gh: the one published release (retired preview tag v0.0.1) was presented as stable Latest — corrected to a prerelease with honest retired-tag notes; the re-run `audit-releases.py --live` passes against the 1.0.0 authority, and public download surfaces (README, repo description) carry no false capability claims. |
...
| 6 | ✅ `Complete` | GitHub-runner CI first-run verified green end-to-end (run 34061368314, all six jobs incl. the browser suites on a live stack with pristine resets). Five runner-only defects were found and fixed on the way: the fmt -p crash, missing Linux WebView deps for the unified workspace, the ESM/CJS clash in the kernel runtime test, stack resets dropping the build stamp, and a pre-restructure workspace map in the architecture test. |
...
| 11 | 🟡 `Partial` | SEC-ASSURE-004 complete (every workflow action SHA-pinned, machine-enforced). SEC-ASSURE-005 complete: seeded kernel fuzz boundaries with a retained regression corpus locally AND a nightly runner job with a rotating seed. SEC-ASSURE-003 CI half complete: a security-scan workflow (push/weekly/nightly) runs secret scanning, cargo-audit (vulnerabilities deny; unmaintained notices ignored with crate-mapped rationale), a 544-dependency machine-readable license inventory with a forbidden-license policy, and the pinning lint — first runner run green (run 34063175852). Remaining: the sequence-4-gated assessment items (reviewer appointment is the owner's step), reference-hardware PERF acceptance, and the soak/Beta/Stable decision. |
